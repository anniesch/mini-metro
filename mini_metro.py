...
class MiniMetroGame:
		...
		for r in range(self.dim + 1):
			for c in range(self.dim + 1):
				...
				if c + 1 != self.dim + 1:
					...
					self.tracks[(cur, curBelow)] = (Track.NoTrack, 0) # second value refers to number of times that the track is used
				if r + 1 != self.dim + 1:
					curRight = enum_point(r + 1, c, self.dim)
					self.tracks[(cur, curRight)] = (Track.NoTrack, 0)
	...
	def addLine(self):
		...
		newLine = len(self.lines) + 1 # it's one indexed
	...
	def addTrackToLine(self, line, point1, point2):
		...
		if self.tracks[track][0] == Track.Track:
			self.tracks[track] = (Track.Track, self.tracks[track][1] + 1)
			self.lines[line].add(track)
			self.totalTracksCovered += 1
		else:
			self.tracks[track] = (Track.Track, 1) # if it's already a track, that's aight
			self.lines[line].add(track)
	...
	def removeTrackFromLine(self, line, point1, point2):
		...
		if self.tracks[track][1] == 1:
			self.tracks[track] = (Track.NoTrack, 0)
	...
	def addStation(point, station_type):
		...
		if station_type == Station.Triangle:
			self.triangleStations.add(point)
