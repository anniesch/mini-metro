...
import collections
import game
from game import AllTrains, WINDOW_SIZE, loadGrid
...
class MiniMetroGame:
	def __init__(self, dim=3, n_types=2):
		...
		self.maxNumLines = ((self.dim + 1) ** 2) - 1
		...
		self.window = GraphWin("MiniMetro", WINDOW_SIZE + 400, WINDOW_SIZE)
		self.block_size = loadGrid(self.window, 4)
		self.allTrains = AllTrains(self.window, self.block_size)
		self.station_pairs_to_color = {}
	...
	def addStation(self, point, station_type):
		...
		if station_type == Station.Triangle:
			...
			self.squareStations.add(point)
		self.allTrains.addStation(int(point[0]), int(point[1]), station_type)
		return 1
	...
	def createLine(self, first_station, second_station): # same as constructLine
		...
		for i in range(len(line)):
			tracks.append(line[i][0])
		tracks.append(line[-1][1])
		self.station_pairs_to_color[pair] = self.allTrains.createNewLine(tracks)
	...
	def removeLine(self, first_station, second_station):
		...
		self.allTrains.removeLine(self.station_pairs_to_color[pair])
		del self.station_pairs_to_color[pair]
	...
	def add_passenger(self):
		...
		if type_point == Station.Triangle:
			...
			self.allTrains.addPassengersToStation(point[0], point[1], 1, 0, 0)
		elif type_point == Station.Circle:
			self.circlePassengers.add(point)
			self.allTrains.addPassengersToStation(point[0], point[1], 0, 1, 0)
		elif type_point == Station.Square:
			self.squarePassengers.add(point)
			self.allTrains.addPassengersToStation(point[0], point[1], 0, 0, 1)
